...
void lpar_cec_fill(struct d204phyhdr *phyhdr)
{
...
void lpar_cec_cpu_fill(struct d204phycpu *phycpu)
{
	struct int_cpu *cpu;
	char cpuid[TMPSIZE];
...
void lpar_this(struct d204timehdr *timehdr)
{
	char id[10];
	struct d204lparhdr *lparhdr;
...
void lpar_read(struct d204hdr **hdr, struct d204timehdr **timehdr)
{
	int fd;
	void *buf;
	size_t rc;
...
void lpar_name(struct d204lparhdr *lparhdr, char *name)
{
...
void *lpar_sys_fill(struct int_sys *sys, struct d204lparhdr *lparhdr)
{
...
void *lpar_cpu_fill(struct int_cpu *cpu, struct d204lparcpu *lparcpu)
{
	//printf("\t\tCPU TIME: %Ld\n", lparcpu->lp_time);
